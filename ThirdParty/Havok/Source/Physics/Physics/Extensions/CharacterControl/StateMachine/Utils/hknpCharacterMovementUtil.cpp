...
void HK_CALL hknpCharacterMovementUtil::calculateMovement( const hknpMovementUtilInput& input, hkVector4& velocityOut )
{
	//
	...
	hkVector4 diff;
	{
		if ( input.m_desiredVelocitySpace == hknpMovementUtilInput::SURFACE_SPACE )
		{
		...
		{
			hkSimdReal len = diff.lengthSquared<3>();
			hkSimdReal tmp;
			if ( len.getReal() * input.m_gain * input.m_gain > input.m_maxVelocityDelta * input.m_maxVelocityDelta )
			{
				tmp.setFromFloat( input.m_maxVelocityDelta );
				tmp.mul( len.sqrtInverse() );
			}
			else
			{
				tmp.setFromFloat( input.m_gain );
			}
			diff.mul( tmp );
		}
	}

...
#ifndef HKNP_CHARACTER_MOVEMENT_UTIL_H
#define HKNP_CHARACTER_MOVEMENT_UTIL_H
...
class hknpCharacterMovementUtil
{
	public:

		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_PHYSICS, hknpCharacterMovementUtil );

		/// The struct passed as input to calculate the new character velocity.
		struct hknpMovementUtilInput
		{
			HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_PHYSICS, hknpCharacterMovementUtil::hknpMovementUtilInput );
			...
			// Ctor.
			hknpMovementUtilInput() : m_desiredVelocitySpace(DEFAULT_SPACE) {}
		...
		static void HK_CALL calculateMovement( const hknpMovementUtilInput& input, hkVector4& velocityOut );
};

#endif // HKNP_CHARACTER_MOVEMENT_UTIL_H

...
// Generated from 'Physics/Constraint/Motor/Position/hkpPositionConstraintMotor.h'
...
const hkClass& HK_CALL hkpPositionConstraintMotor::staticClass()
{
	return hkpPositionConstraintMotorClass;
}
HK_COMPILE_TIME_ASSERT2( \
	sizeof(hkIsVirtual(static_cast<hkpPositionConstraintMotor*>(0))) == sizeof(hkBool::CompileTimeTrueType), \
	REFLECTION_PARSER_VTABLE_DETECTION_FAILED );
static void HK_CALL finishLoadedObjecthkpPositionConstraintMotor(void* p, int finishing = 1)
{
	hkFinishLoadedObjectFlag f;
	f.m_finishing = finishing;
	new (p) hkpPositionConstraintMotor(f);
}
static void HK_CALL cleanupLoadedObjecthkpPositionConstraintMotor(void* p)
{
	static_cast<hkpPositionConstraintMotor*>(p)->~hkpPositionConstraintMotor();
}
static const void* HK_CALL getVtablehkpPositionConstraintMotor()
{
	#if HK_LINKONCE_VTABLES==0
	#if HK_HASHCODE_VTABLE_REGISTRY==1
	return ((const void*)(typeid(hkpPositionConstraintMotor).hash_code()));
	#else
	return ((const void*)(typeid(hkpPositionConstraintMotor).name()));
	#endif
	#else
	union { HK_ALIGN16(void* ptr); char buf[sizeof(hkpPositionConstraintMotor)]; } u;
	hkFinishLoadedObjectFlag f;
	new (u.buf) hkpPositionConstraintMotor(f);
	return u.ptr;
	#endif
}
extern const hkTypeInfo hkpPositionConstraintMotorTypeInfo;
const hkTypeInfo hkpPositionConstraintMotorTypeInfo(
	"hkpPositionConstraintMotor",
	"!hkpPositionConstraintMotor",
	finishLoadedObjecthkpPositionConstraintMotor,
	cleanupLoadedObjecthkpPositionConstraintMotor,
	getVtablehkpPositionConstraintMotor(),
	sizeof(hkpPositionConstraintMotor)
	);
#endif

...
hkMonitorAllocator::hkMonitorAllocator(hkMemoryAllocator* child, const char* mtag)
: m_child(child), m_monitorTag(mtag)
{
	HK_ASSERT(0x54e4a336, mtag[0] == 'm');
}

void* hkMonitorAllocator::blockAlloc( int size )
{
	void* p = m_child->blockAlloc(size);
	HK_MONITOR_MEMORY_ALLOC(m_monitorTag, p, size);
	return p;
}
void hkMonitorAllocator::blockFree( void* p, int size )
{

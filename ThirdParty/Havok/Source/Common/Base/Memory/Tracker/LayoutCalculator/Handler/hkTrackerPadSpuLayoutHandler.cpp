...
void hkTrackerPadSpuLayoutHandler::getReferences(
...
{
	const hkPadSpu<void*>* padSpu = static_cast<const hkPadSpu<void*>*>(curData);
	const hkTrackerTypeTreeNode* contentType = curType->m_contains;
	const void* data = &padSpu->ref();

	layoutCalc->getReferencesRecursive(curBlock, data, contentType, newBlocks);
}

hk_size_t hkTrackerPadSpuLayoutHandler::getSize(
	const hkTrackerTypeTreeNode*, // unused
	hkTrackerLayoutCalculator* ) // unused
{
	return sizeof(hkPadSpu<void*>);
}

...
void hkMatrix6fAdd(hkMatrix6f& aOut, const hkMatrix6f& b)
{
	aOut.add(b);
}

void hkMatrix6fSub(hkMatrix6f& aOut, const hkMatrix6f& b)
{
	aOut.sub(b);
}

void hkMatrix6fSetMul(hkMatrix6f& out, const hkMatrix6f&a, const hkMatrix6f&b)
{
	out.setMul(a,b);
}

void hkMatrix6fSetMulV(hkVector8f& out, const hkMatrix6f&a, const hkVector8f& b)
{
	out._setMul6(a,b);
}

void hkMatrix6fSetTranspose(hkMatrix6f& out, const hkMatrix6f&in)
{
	out.setTranspose(in);
}

void hkMatrix6fSetInvert(hkMatrix6f& out, const hkMatrix6f& in )
{
	out.setInvert(in);
}


void hkMatrix6f::setInvert( const hkMatrix6f& tmpIn )
{
	hkMatrix6f& out = *this;
	hkMatrix6f in = tmpIn;
	out.setIdentity();
		...
		{
			{
				...
				hkSimdFloat32 multiplier; multiplier.setFromFloat( 1.0f / in.m_m[row2][row2](row3,row3) );
			...
			for (int col2 = 0; col2 < 2; col2++)
			{
				for (int col3 = 0; col3 < 3; col3++)
				{
					if (col2 != row2 || col3 != row3)
					{
						...
						hkSimdFloat32 multiplier; multiplier.setFromFloat( - in.m_m[row2][col2](row3,col3) );

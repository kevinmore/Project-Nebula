...
void hkSweptTransformf::approxTransformAt( hkSimdFloat32Parameter time, hkTransformf& transformOut ) const
{
#if defined(HK_PLATFORM_PS3_SPU)
	// on SPU force outline
	const hkSimdFloat32 dt = getInterpolationValue(time);
...
void hkSweptTransformf::approxTransformAt( hkTime time, hkTransformf& transformOut ) const
{
#if defined(HK_PLATFORM_PS3_SPU)
	// on SPU force outline
	const hkSimdFloat32 dt = hkSimdFloat32::fromFloat(getInterpolationValue(time)); // late transition
...
void hkSweptTransformf::initSweptTransform( hkVector4fParameter position, hkQuaternionfParameter rotation )
{
	// need the 0 here?

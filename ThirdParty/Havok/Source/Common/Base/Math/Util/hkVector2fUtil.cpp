...
namespace
{
 	struct IndexedLess
 	{
		IndexedLess( const hkArrayBase<hkVector2f>& a ) : m_a(a) {  }
		hkBool32 operator()( int i, int j )
		{
			const hkVector2f& pi = m_a[i];
			const hkVector2f& pj = m_a[j];
			return pi.x < pj.x || (pi.x == pj.x && pi.y < pj.y);
		}
		const hkArrayBase<hkVector2f>& m_a;
	};
...
void HK_CALL convexHullSimplePolyline( const hkArrayBase<hkVector2f>& line, hkArray<int>& indicesOut )
{
	...
	hkVector2f bot0 = line[ deque.bot(0) ];
	hkVector2f bot1 = line[ deque.bot(1) ];
	hkVector2f top0 = line[ deque.top(0) ];
	hkVector2f top1 = line[ deque.top(1) ];
...
hkResult HK_CALL convexHullIndices( const hkArrayBase<hkVector2f>& originalPoints, hkArrayBase<int>& indicesOut )
{
	...
	hkLocalBuffer<hkVector2f> points( NPOINT );
...

	{
		...
		{
			const hkVector2f& p = originalPoints[ originalIndices[si] ];
	...
	hkLocalArray<hkVector2f> pointsOut( NPOINT );
	...
	// get start and end walking indices
	{
		...
		minXmaxY -= 1;
		{
			hkFloat32 maxXpos = points[NPOINT-1].x;
			int i;
	...
	{
		hkVector2f minXminYpoint = points[minXminY];
		hkVector2f maxXminYpoint = points[maxXminY];
	...
	// walk backwards along top.
	{
		...
		hkVector2f maxXmaxYpoint = points[NPOINT-1];
		hkVector2f minXmaxYpoint = points[minXmaxY];
...
hkBool HK_CALL edgesIntersect( const hkVector2f& a, const hkVector2f& b, const hkVector2f& c, const hkVector2f& d )
{
	...
	hkFloat32 denom = (b.x - a.x) * (d.y - c.y) - (b.y - a.y) * (d.x - c.x);
	hkFloat32 rNum = (a.y - c.y) * (d.x - c.x) - (a.x - c.x) * (d.y - c.y);
	hkFloat32 sNum = (a.y - c.y) * (b.x - a.x) - (a.x - c.x) * (b.y - a.y);
...
hkFloat32 computeObb(const hkArrayBase<hkVector2f>& points, hkVector2f& centerOut, hkVector2f& e0Out, hkVector2f& e1Out, hkResult& resOut)
{
	...
	hkFloat32 minArea = HK_FLOAT_MAX;

	for (int i = 0, j = hullIndices.getSize()-1; i < hullIndices.getSize(); j = i, i++)
	{
		hkVector2f e0; e0.setSub(points[ hullIndices[i] ], points[ hullIndices[j] ]);
		if (e0.normalizeWithLength() == 0.f)
			continue;

		hkVector2f e1; e1.setPerp(e0);

		hkFloat32 min0 = 0.f, min1 = 0.f, max0 = 0.f, max1 = 0.f;
		for (int k = 0; k < hullIndices.getSize(); k++)
		{
			hkVector2f d; d.setSub(points[ hullIndices[k] ], points[ hullIndices[j] ]);
			hkFloat32 dot = d.dot(e0);
		...
		hkFloat32 area = (max0 - min0) * (max1 - min1);
...
int HK_CALL getStackSizeInBytesRequiredForConvexHullIndices(int inputPointsSize)
{
	return inputPointsSize * 
		( sizeof(hkVector2f)	+ //hkLocalBuffer<hkVector2f> points( NPOINT );
		 sizeof(int)	+ // hkLocalBuffer<int> originalIndices( NPOINT );
		 sizeof(hkVector2f)	// hkLocalArray<hkVector2f> pointsOut( NPOINT );
	);
}
#endif
